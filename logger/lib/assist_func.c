...
unsigned char FAULT_Mask_Check(unsigned short data_addr)
{
    ...
    for(i = 0; ((i < sys_cfg->fault_ignore_num) && (i < FAULT_IGNORE_NUM)); i++)
    {
        if(sys_cfg->fault_ignore[i] == data_addr) //屏蔽的点位列表中存在 该点位
        {
            ret = 1;
            break;
        }
...
unsigned char LOGIC_Mask_Check(unsigned short data_addr)
{
    ...
    for(i = 0; ((i < sys_cfg->logic_ignore_num) && (i < LOGIC_IGNORE_NUM)); i++)
    {
        if(sys_cfg->logic_ignore[i] == data_addr) //屏蔽的点位列表中存在 该点位
        {
            ret = 1;
            break;
        }

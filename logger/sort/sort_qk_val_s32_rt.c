...
static boolean_T findPivot(int32_T *dataArray, int_T iQS, int_T jQS, int_T *pivot)
{
    int_T   mid = (iQS+jQS)>>1;
    int_T   kQS;
    int32_T  a, b, c;
...
static int_T partition(int32_T *dataArray, int_T iQS, int_T jQS, int_T pivot)
{
...
LIBMW_SRC_API void MWDSP_Sort_Qk_Val_S32(int32_T *dataArray, int_T iQS, int_T jQS)
{
    int_T pivot;
    if (findPivot(dataArray, iQS, jQS, &pivot)) {
        int_T kQS = partition(dataArray, iQS, jQS, pivot);
        MWDSP_Sort_Qk_Val_S32(dataArray, iQS, kQS-1);
        MWDSP_Sort_Qk_Val_S32(dataArray, kQS, jQS);
    }
}

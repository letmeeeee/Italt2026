...
static boolean_T findPivot(int16_T *dataArray, int_T iQS, int_T jQS, int_T *pivot)
{
    int_T   mid = (iQS+jQS)>>1;
    int_T   kQS;
    int16_T  a, b, c;
...
static int_T partition(int16_T *dataArray, int_T iQS, int_T jQS, int_T pivot)
{
...
LIBMW_SRC_API void MWDSP_Sort_Qk_Val_S16(int16_T *dataArray, int_T iQS, int_T jQS)
{
    int_T pivot;
    if (findPivot(dataArray, iQS, jQS, &pivot)) {
        int_T kQS = partition(dataArray, iQS, jQS, pivot);
        MWDSP_Sort_Qk_Val_S16(dataArray, iQS, kQS-1);
        MWDSP_Sort_Qk_Val_S16(dataArray, kQS, jQS);
    }
}

...
#define CheckDINPort(p) { \
	if ( (p) < 0 || (p) >= MAX_DIN_PORT ) \
		return DIO_ERROR_PORT;	\
}
#define CheckDOUTPort(p) { \
	if ( (p) < 0 || (p) >= MAX_DOUT_PORT ) \
		return DIO_ERROR_PORT;	\
}
#define CheckState(s) { \
...
static din_event_t	din_event[MAX_DIN_PORT]={

...
#define PCS_TIMEOUT_CNT         (1700)    //PCS通信超时时间(PCS 17s判定离线，1700*10ms)
#define BMS_TIMEOUT_CNT         (60000)   //BMS通信超时时间(BMS 600s判定离线，60000*10ms)
...
#define MEASU_TIMEOUT_CNT       (3000)    //测控装置通信超时时间(METER 30s判定离线，3000*10ms)
#define ZERO_TIMEOUT_CNT        (6000)    //零功率超时时间(60s，6000*10ms)
#define GAS_TIMEOUT_CNT         (3000)    //气体传感器通信超时时间(30s判定离线，3000*10ms)
#define EMS_TIMEOUT_CNT         (18000)    //EMS通信超时时间(180s判定离线，18000*10ms)
...
//!定时器变量
extern unsigned int Timer_Tick;                             //系统TICK
extern unsigned int volatile TIMEOUT_TIMER[TIMER_CNT];               //定时器定义
